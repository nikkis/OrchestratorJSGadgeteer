...
namespace JDI.WebSocket.Client
{
	internal class WSFrame : IDisposable
	{
		...
		public static WSFrame CreateFrame(WSFrameType opCode, bool isMasked, byte[] payLoad)
		{
            ...
            /*
            DebugEx.WriteLine(" ########################## ---------------------------- ");
            DebugEx.WriteLine("Val:");
            DebugEx.WriteLine(opCode.ToString());
            string m = GadgeteerOrchestratorJSClient.ByteExtensions.ToHexString(payLoad);
            DebugEx.WriteLine(m);
            DebugEx.WriteLine(" ########################## ---------------------------- ");
            */


            // HalliOlli -function's CryptoUtils.GetRandomBytes randomly causes an exception, so giving false for isMasked parameter always
            int i = 0;
            while (i < 10)
            {
                ++i;
                ...
                try
                {
                    return HarriOlli(opCode, false, payLoad);
                    break;
                }
                catch (Exception eeeXXX)
                {
                    DebugEx.WriteLine(" ########################## ---------------------------- EEEEEE");
                    DebugEx.WriteLine(eeeXXX.ToString());
                    DebugEx.WriteLine(" ########################## ---------------------------- EEEEEE");
                    if(i > 2) 
                    {
                        //throw(eeeXXX);
